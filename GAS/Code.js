...
function postShiftReport(){
  ...
  for (var i = 0; i < numRows; i++){
    for (j=0; j<headerValues.length; j++) {
      ...
      if (lastStageEndHours<=8){dayNight="Night Shift"}else{dayNight="Day Shift"}
  ...
  for (k=0;k<atpValues.length;k++){
    ...
    if(comb == ""){continue}else{latestATPcomb=atpValues[k][2];}
